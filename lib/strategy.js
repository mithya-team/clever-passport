...
OAuth2Strategy.prototype.userProfile = function (accessToken, done) {
    ...
    async function getUserInfo(link, user, options) {
        ...
        if (info.data?.roles?.teacher) {
            ...
            await getStudentsForTeacher(user.id, options);
        }
    }
    ...
    async function getStudentsForTeacher(user_id, options) {
        const response = await fetch(`https://api.clever.com/v3.0/users/${user_id}/mystudents`, options);
        const info = await response.json();
